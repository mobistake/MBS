...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if (nHeight > 10000)	return 3000 * COIN;
    if (nHeight > 1)		return 2000 * COIN;
    return 10000 * COIN;
}

CAmount CMasternode::GetBlockValue(int nHeight)
{
    if (nHeight > 6301835)	return 0   * COIN;
    if (nHeight > 3000000)	return 0.5 * COIN;
    if (nHeight > 2000000)	return 1   * COIN;
    if (nHeight > 1000000)	return 3   * COIN;
    if (nHeight > 500000)	return 5   * COIN;
    if (nHeight > 100000)	return 4   * COIN;
    if (nHeight > 50000)	return 2   * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 MBS tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 MBS tx -> 1 confirmation
