...
const char * const PIVX_CONF_FILENAME = "mbs.conf";
const char * const PIVX_PID_FILENAME = "mbs.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Mobistake only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\mbs
// Windows >= Vista: C:\Users\Username\AppData\Roaming\mbs
// Mac: ~/Library/Application Support/mbs
// Unix: ~/.mbs
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mobistake";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\mbsParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\mbsParams
    // Mac: ~/Library/Application Support/mbsParams
    // Unix: ~/.mbs-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MobistakeParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "MobistakeParams";
#else
    // Unix
    return pathRet / ".mbs-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty mbs.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Mobistake config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override mbs.conf
