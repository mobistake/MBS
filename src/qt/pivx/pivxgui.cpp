...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Mobistake client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Mobistake"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Mobistake"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
