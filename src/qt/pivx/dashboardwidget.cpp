    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of MBS earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 MBS");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("MBS")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MBS, zMBS
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(14, 178, 216));
	set1->setColor(QColor(0, 70, 119));
