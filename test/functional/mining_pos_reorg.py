...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected MBS and zMBS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zMBSsupply']:
                assert_equal(node['zMBSsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check MBS and zMBS supply at the beginning
        # ------------------------------------------
        # zMBS supply: 2 coins for each denomination
        ...
        # MBS supply: block rewards minus burned fees for minting
        ...
        # Verify that MBS and zMBS supplies were properly updated after the spends and reorgs
        self.log.info("Check MBS and zMBS supply...")
